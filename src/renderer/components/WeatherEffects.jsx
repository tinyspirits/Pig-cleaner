...
import Wildlife from './Wildlife' // Import hệ sinh thái (Cá, Chim) vào
...
export default function WeatherEffects({ weather, poolMode = false, effectsEnabled = true, cameraFollowsPig = true }) {
  ...
  // Xử lý mực nước và tuyết dâng
  useEffect(() => {
    const isHeavyRain = weather?.condition === 'thunderstorm' || poolMode
    const isSnowing = weather?.condition === 'snow'

    const interval = setInterval(() => {
      setWaterLevel(prev => isHeavyRain ? Math.min(50, prev + 1.5) : Math.max(0, prev - 3))
      setSnowLevel(prev => isSnowing ? Math.min(1, prev + 0.1) : Math.max(0, prev - 0.2))
    }, 1000)
  ...
  return (
    <div ref={containerRef} style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 50 }}>
      ...
      {/* Lớp sinh vật (Cá và Chim) được import từ Component mới */}
      <Wildlife poolMode={poolMode} waterLevel={waterLevel} />
    </div>
  )
}