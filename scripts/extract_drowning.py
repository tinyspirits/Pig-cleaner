...
out_dir = 'src/renderer/assets/sprites/drowning_frames'
...
print(f"Found {len(all_rects)} drowning frames!")
...
for i, (x, y, w, h) in enumerate(all_rects):
    ...
    cv2.imwrite(f'{out_dir}/drowning_{i}.png', rgba)
...
for i in range(len(all_rects)):
    pimg = cv2.imread(f'{out_dir}/drowning_{i}.png', cv2.IMREAD_UNCHANGED)
    if pimg is not None:
        ph, pw = pimg.shape[:2]
...
cv2.imwrite(f'{out_dir}/all_drowning_pigs.png', grid)
print("Created map all_drowning_pigs.png!")
